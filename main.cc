...
struct sss_state {
    ...
    bool ignore_childs() {
        ...
        if (!ignore_c && father != nullptr) {
            ignore_c |= father->ignore_childs();
        }
...
int sss_star(state_t n, int color, int boud) {
    ...
    while (true) {
        ...
        if (state->father != nullptr && state->father->ignore_childs()){
            continue;
        }
